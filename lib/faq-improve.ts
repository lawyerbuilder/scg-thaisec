...
RULES
- Stay grounded EXCLUSIVELY in the source regulation. Do not introduce facts that aren't \
in the regulation.
- Cite specific Thai law sections (e.g. "มาตรา 103 พ.ร.บ.บริษัทมหาชนฯ") when the \
regulation contains them.
- Keep answers concise: 2-5 sentences. Prefer specific numbers/dates/percentages over \
generalities.
- Output BOTH Thai and English for question and answer. Translations must preserve \
legal precision.
- 'improvements_made' must accurately describe what you changed. Do not invent improvements.
- If the source regulation doesn't actually cover the question well, raise that in \
'warnings' with confidence='low' — don't pretend to answer.
...
export async function improveFaq(req: ImprovementRequest): Promise<ImprovementResponse> {
  ...
  const response = await groq.chat.completions.create({
    model: MODEL,
    messages: [{ role: "user", content: buildPrompt(req, reg) }],
    response_format: {
      type: "json_schema",
